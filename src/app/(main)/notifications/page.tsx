...
import { Bell, ArrowLeft, Sparkles, ShieldAlert } from "lucide-react";
    ...
    <div className="max-w-xl mx-auto px-4 py-8 flex flex-col items-center justify-center min-h-[calc(100vh-140px)] text-center">
      ...
      >
        <div className="size-20 rounded-3xl bg-gradient-to-tr from-violet-600/30 via-primary/20 to-indigo-500/30 border border-primary/40 flex items-center justify-center shadow-[0_0_30px_rgba(139,92,246,0.25)]">
          <Bell size={36} className="text-primary animate-pulse" />
        </div>
        <div className="absolute -top-1 -right-1 flex size-6 items-center justify-center rounded-full bg-[#EF4444] text-white text-[10px] font-bold shadow-md">
          !
        </div>
      </motion.div>
      ...
      <p className="text-sm text-muted-foreground max-w-sm mb-6 leading-relaxed">
        Cette partie est actuellement <strong className="text-white">en cours de développement 🚧</strong>.
        Tu pourras bientôt recevoir les alertes de nouveaux épisodes, les interactions sur tes posts et les défis de tes amis en temps réel !
      </p>

      {/* ── Aperçu des filtres futurs (Écran 6) ── */}
      <div className="w-full max-w-xs p-4 rounded-2xl border border-border/80 bg-card/60 backdrop-blur-sm mb-6 text-left space-y-2.5">
        ...
        <ul className="text-xs text-foreground/80 space-y-2">
          <li className="flex items-center gap-2">
            ...
            Alertes push PWA pour les sorties d'épisodes
          </li>
          <li className="flex items-center gap-2">
            <span className="size-1.5 rounded-full bg-primary" />
            Mentions, likes & réponses de la communauté
          </li>
          <li className="flex items-center gap-2">
            <span className="size-1.5 rounded-full bg-primary" />
            Nouveaux followers et défis otakus
          </li>
        </ul>
      </div>
